...
export function AddTransactionModal() {
  ...
  const selectedItem = CATEGORIES.find(c => c.name === selectedCategory) ?? CATEGORIES[0];
                ...
                <div className="absolute top-[calc(100%+8px)] left-0 right-0 z-50 bg-surface border border-secondary/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 max-h-72 overflow-y-auto">
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 text-left font-semibold transition-colors hover:bg-brand-teal/10 ${
                        selectedCategory === cat.name ? 'text-brand-teal bg-brand-teal/10' : 'text-primary'
                      }`}
                    >
                      <span className="text-lg w-7 text-center">{cat.emoji}</span>
                      <span className="text-sm">{cat.name}</span>
                      {selectedCategory === cat.name && (
                        <span className="ml-auto w-2 h-2 rounded-full bg-brand-teal shrink-0" />
                      )}
                    </button>
                  ))}
                </div>
